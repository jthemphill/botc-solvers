...
function removeNameFromClaim(claim: Claim, name: string): Claim | undefined {
  ...
  switch (claim.type) {
    ...
    case "Balloonist":
      return {
        ...claim,
        differentCharacterTypePairs: claim.differentCharacterTypePairs.filter(
          ([l, r]) => l !== name && r !== name,
        ),
      };
    default:
...
export function reducer(state: PuzzleDoc, action: PuzzleAction): PuzzleDoc {
  switch (action.type) {
    ...
    case "removePlayer": {
      ...
      const you = state.you?.name === name ? undefined : state.you;
      return { ...state, players, claims, you };
    }
    case "renamePlayer": {
      ...
      const you = state.you?.name === oldName ? { ...state.you, name: action.name } : state.you;
      return { ...state, players, claims, you };
    }
    case "movePlayer": {
